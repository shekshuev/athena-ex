defmodule AthenaWeb.LearnLive.ExamTest do
  ...
  describe "Exam Navigation & Autosave" do
    ...
    test "jumps to specific question and renders finish button on last question when all answered",
      ...
      lv |> form("#exam-quiz-#{q1_id}", %{"answer" => "4"}) |> render_change()

      lv |> element("button[phx-click='jump_to'][phx-value-index='1']") |> render_click()
      lv |> form("#exam-quiz-#{q2_id}", %{"answer" => "opt1"}) |> render_change()

      _html = render_click(lv, "jump_to", %{"index" => "2"})

      lv |> form("#exam-quiz-#{q3_id}", %{"answer" => "My essay"}) |> render_change()
  ...
  describe "Exam completion" do
    test "finishing a fully auto-graded exam that passes the gate marks the block completed", %{
      ...
      lv |> form("#exam-quiz-#{q1_id}", %{"answer" => "4"}) |> render_submit()
      lv |> form("#exam-quiz-#{q2_id}", %{"answer" => "6"}) |> render_submit()
    ...
    test "finishing a fully auto-graded exam that fails the gate does not complete the block", %{
      conn: conn,
      course: course,
      section: section,
      user: user
      ...
      lv |> form("#exam-quiz-#{q1_id}", %{"answer" => "wrong"}) |> render_submit()
