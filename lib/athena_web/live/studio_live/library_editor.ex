defmodule AthenaWeb.StudioLive.LibraryEditor do
  ...
  def handle_event("add_quiz_pair", _, socket) do
    case can_edit?(socket) do
      true ->
        block = socket.assigns.block
        content_map = normalize_content(block.content || %{})
  ...
  def handle_event("remove_quiz_pair", %{"pair_id" => pair_id}, socket) do
    case can_edit?(socket) do
      true ->
        block = socket.assigns.block
        content_map = normalize_content(block.content || %{})
        ...
        update_and_assign(socket, block, %{"content" => Map.put(content_map, "pairs", pairs)})
  ...
  def handle_event("update_quiz_content", params, socket) do
    case can_edit?(socket) do
      true ->
        ...
        content_map =
          if pairs = params["pairs"],
            do: Map.put(content_map, "pairs", parse_quiz_pairs(pairs)),
            else: content_map
