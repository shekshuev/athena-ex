defmodule AthenaWeb.StudioLive.CourseShareComponent do
  @moduledoc """
  LiveComponent for managing course visibility and sharing access.
  """
  ...
  defp load_shares(socket, course) do
    ...
    socket
    |> assign(shares: enriched_shares, is_public: course.is_public)
  end
  ...
  def handle_event("toggle_public", %{"is_public" => is_public_str}, socket) do
    ...
    case Content.toggle_course_public(
           socket.assigns.current_user,
           socket.assigns.course,
           is_public
         ) do
      {:ok, updated_course} ->
        send(self(), {__MODULE__, {:updated, updated_course}})
        {:noreply, assign(socket, is_public: updated_course.is_public)}
  ...
  def handle_event("change_role", %{"account_id" => account_id, "role" => new_role}, socket) do
    ...
    case Content.share_course(
      ...
      {:ok, _share} ->
        socket = load_shares(socket, socket.assigns.course)
        {:noreply, socket}
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-base-content/70">
              {gettext("Anyone on the platform can view and clone this course.")}
            </p>
          </div>
          <.form
            for={nil}
      ...
      <div class="space-y-2">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span class="font-bold">{share.login}</span>
          </div>
