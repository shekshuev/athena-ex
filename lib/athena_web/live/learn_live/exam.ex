defmodule AthenaWeb.LearnLive.Exam do
  ...
  # here (see its `start_exam_for_block/2`), so a quiz_exam/ticket_exam
  # started inside a test run stays inside the sandbox instead of taking
  # the browser to a real page under the instructor's own account. Nothing
  # in this LiveView may navigate the real browser while nested; `exit_exam/4`
  # messages the parent to swap back to `Player` instead.
  def mount(
        ...
        {:already_completed, _submission} ->
          {:ok,
           exit_bail(
             ...
             return_to,
             :info,
             gettext("You've already completed this assessment session.")
           )}
